...
$_SESSION ['regno']= $regno;
    ...
    header("Location:http://localhost/Lib/ProcessUpdateDetails.php");
	mysqli_close($con);
}
?>
...
<div class="container" >
<form class="form-horizontal" action="UpdateStudentDetails.php" method="get">
  
  <div class="form-group">
      <label class="control-label col-sm-2" for="regno">Register No:</label>
      <div class="col-sm-6">
		<select  class="form-control" name="regno">
       <option value=""> --------Register No-------- </option> 
    <?php include 'connection.php';
         $dd_res9=mysqli_query($con,"SELECT Distinct RegNo FROM student");
		 
         while($row10=mysqli_fetch_row($dd_res9))
         { 
	        // echo "<option value='" . $row['IssuedBook'] ."'>" . $row['IssuedBook'] ."</option>";
               echo "<option value='$row10[0]'> $row10[0] </option>";
         }
     ?>
	 </div>
    </div>
	  <br>
		<input type="submit"  class="btn btn-primary btn-xs" name="submitedits" value="Edit Student">
		</form>
